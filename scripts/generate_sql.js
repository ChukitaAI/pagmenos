...
const raw = fs.readFileSync('seed_data.json', 'utf8');
...
for (const c of data.categories) {
    sql += \`INSERT INTO public.categories (id, name, slug, icon_key, display_order, active) VALUES (gen_random_uuid(), '\${c.name.replace(/'/g, "''")}', '\${c.slug}', '\${c.icon_key}', \${c.display_order}, \${c.active}) ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name, icon_key = EXCLUDED.icon_key, display_order = EXCLUDED.display_order, active = EXCLUDED.active;\n\`;
}

sql += '\\n-- Products\\n';

for (const p of data.products) {
    const promoLink = data.promoProducts.find(x => x.product_id === p.id);
    let promo = null;
    if (promoLink) {
        promo = data.promos.find(x => x.id === promoLink.promotion_id && x.active);
    }
    ...
    sql += \`INSERT INTO public.products (id, category_id, name, slug, description, brand, price_cents, promotional_price_cents, promotion_starts_at, promotion_ends_at, is_in_stock, is_featured, active, image_url) VALUES (gen_random_uuid(), (SELECT id FROM public.categories WHERE slug = '\${catSlug}'), '\${p.name.replace(/'/g, "''")}', '\${p.slug}', \${p.description ? "'" + p.description.replace(/'/g, "''") + "'" : 'NULL'}, \${p.brand ? "'" + p.brand.replace(/'/g, "''") + "'" : 'NULL'}, \${p.base_price_cents}, \${promoPrice}, \${promoStart}, \${promoEnd}, \${p.stock_status === 'in_stock'}, \${p.featured}, \${p.active}, \${p.image_url ? "'" + p.image_url + "'" : 'NULL'}) ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name, category_id = EXCLUDED.category_id, price_cents = EXCLUDED.price_cents, promotional_price_cents = EXCLUDED.promotional_price_cents, is_in_stock = EXCLUDED.is_in_stock, is_featured = EXCLUDED.is_featured, active = EXCLUDED.active, image_url = EXCLUDED.image_url;\n\`;
}
...
fs.writeFileSync('supabase/supabase_setup.sql', sql);
